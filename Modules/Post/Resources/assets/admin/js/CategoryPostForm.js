import CategoryPostTree from './CategoryPostTree';

export default class {
    constructor() {
        let tree = $('.category-tree');

        new CategoryPostTree(this, tree);
    ...
    fetchCategory(id) {
        ...
        $.ajax({
            type: 'GET',
            url: route('admin.category_posts.show', id),
            success: (category) => {
    ...
    update(category) {
        ...
        $('#confirmation-form').attr('action', route('admin.category_posts.destroy', category.id));
    ...
    submit(e) {
        ...
        if (! $('#slug-field').hasClass('hide')) {
            ...
            $('#category-form').attr('action', route('admin.category_posts.update', selectedId));
        }
